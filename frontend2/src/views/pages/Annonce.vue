...
<template>
    <div class="container">
        ...
        <div class="card">
            ...
            <DataView  v-if="!loading" :value="listings" paginator :rows="10">
                    ...
                    <div class="grid grid-nogutter">
                        <div v-for="(item, index) in slotProps.items" :key="index" class="col-12">
                            <div class="flex flex-column sm:flex-row sm:align-items-center p-4 gap-3" :class="{ 'border-top-1 surface-border': index !== 0 }">
                                <router-link
                                    :key="item.id"
                                    :to="{ name: 'annoncesUnique', params: { id: item.id } }"
                                    :id="'listing-banner-image-for-' + item.code"
                                    >
                                    <div class="md:w-10rem relative">
                                        <img class="block xl:block mx-auto border-round w-full" :src="'https://echri.store/uploads/listing_thumbnails/' + item.listing_thumbnail" :alt="item.name" />
                                    ...
                                    <div class="flex flex-row md:flex-column justify-content-between align-items-start gap-2">
                                        ...
                                        <div class="surface-100 p-1" style="border-radius: 30px">
                                            <div class="surface-0 flex align-items-center gap-2 justify-content-center py-1 px-2" style="border-radius: 30px; box-shadow: 0px 1px 2px 0px rgba(0, 0, 0, 0.04), 0px 1px 2px 0px rgba(0, 0, 0, 0.06)">
                                                <span class="text-900 font-medium text-sm">4</span>
...
return `https://echri.store/api/get_listing_url/${listingId}`;
};
...
try {
  ...
  const response = await axios.get(`https://echri.store/search?search_string=&selected_city_id=&selected_category_id=${currentId}`);
  ...
  for (const list of listings.value) {
          const romspec = await axios.get(`https://echri.store/api/get_hotel_spec/${list.id}`);
        //   console.log('Response from Room spec:', romspec.data);
          romspecs.value[list.id] = romspec.data;
    }
  const get_phrases_show_on_map = await axios.get('https://echri.store/api/get_phrase/show_on_map');
  get_phrase_show_on_map.value = get_phrases_show_on_map.data;
  const get_phrase_watch_detail = await axios.get('https://echri.store/api/get_phrase/watch_details');
  get_phrase_watch_details.value = get_phrase_watch_detail.data;
  const get_phrase_feature = await axios.get('https://echri.store/api/get_phrase/featured');
  get_phrase_featured.value = get_phrase_feature.data;
  const this_listing_is_verifi = await axios.get('https://echri.store/api/get_phrase/this_listing_is_verified');
  this_listing_is_verified.value = this_listing_is_verifi.data;


      const responsegeo = await fetch("https://echri.store/home/filter_listings");
  ...
  const get_count = await axios.get('https://echri.store/api/get_country', { listing_id: 2 });
